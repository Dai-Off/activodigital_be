...
class AIInvoiceController {
    constructor() {
        ...
        this.invoiceJobService = new invoiceProcessingJobService_1.InvoiceProcessingJobService();
         ...
         */
        this.extractInvoiceAsync = async (req, res) => {
            try {
                ...
                const { document_url, document_filename, building_id } = req.body;
                ...
                await (0, invoiceProcessingQueue_1.addInvoiceProcessingJob)(job.id);
                res.status(202).json({
                    success: true,
                    job_id: job.id,
        ...
        this.getInvoiceJob = async (req, res) => {
            try {
                const authUserId = req.user?.id;
                if (!authUserId) {
                    res.status(401).json({ error: 'Usuario no autenticado' });
