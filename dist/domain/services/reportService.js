...
class ReportService {
     ...
     */
    async generateReport(options, userAuthId) {
     ...
     */
    async getReports(userAuthId, category) {
        const user = await this.userService.getUserByAuthId(userAuthId);
        if (!user)
            throw new Error("Usuario no encontrado");
     ...
     */
    async gatherBuildingData(buildingIds, userAuthId) {
     ...
     * Excel Generation
     */
    async generateExcel(options, buildings) {
