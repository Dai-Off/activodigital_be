...
class InvitationService {
     ...
     */
    async acceptInvitation(data, userAuthId) {
        try {
     ...
     */
    async getPendingInvitation(email, buildingId) {
        const { data, error } = await this.getSupabase()
            .from('invitations')
            .select(`
        *,
     ...
     */
    async getInvitationsByUser(userAuthId) {
     ...
     */
    async cancelInvitation(invitationId, userAuthId) {
        const user = await this.userService.getUserByAuthId(userAuthId);
        if (!user) {
            throw new Error('Usuario no encontrado');
        }
     ...
     */
    async getCfoAssignmentsForBuilding(buildingId) {
        const { data, error } = await this.getSupabase()
     ...
     */
    async getCfoAssignmentsForUser(userAuthId) {
        const user = await this.userService.getUserByAuthId(userAuthId);
        if (!user) {
            throw new Error('Usuario no encontrado');
        }
    ...
    // Métodos auxiliares
    async getBuildingForInvitation(buildingId) {
        const { data, error } = await this.getSupabase()
            .from('buildings')
