...
class NotificationService {
     ...
     */
    async getBuildingNotifications(buildingId, filters = {}) {
     ...
     */
    async getUserReadNotificationIds(userId) {
     ...
     */
    async deleteOldNotifications(buildingId, daysOld = 30) {
    ...
    async markAllAsReadForUser(userId) {
        try {
            ...
            const unreadNotifications = allUserNotifications.filter((notification) => !readIds.has(notification.id));
     ...
     */
    async checkIfRead(userId, notificationId) {
        const { data, error } = await this.getSupabase()
            .from("notification_reads")
            .select("user_id")
            .eq("user_id", userId)
