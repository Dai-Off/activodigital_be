...
class RegulatoryAuditService {
    ...
    async getRegulatoryAudit(buildingId, userAuthId) {
        ...
        const currentState = {
            energy_class: certificate?.rating || building?.custom_data?.calificacion || 'G',
            consumption_kwh_m2_year: certificate?.primary_energy_kwh_per_m2_year || building?.custom_data?.costeEnergetico || 0,
            emissions_kg_co2_m2_year: certificate?.emissions_kg_co2_per_m2_year || building?.custom_data?.emisiones || 0,
            heating_demand: building?.custom_data?.demandaCalefaccion,
            cooling_demand: building?.custom_data?.demandaRefrigeracion,
        };
        ...
        const hasManualData = !!building?.custom_data?.calificacion;
        ...
        const hasCEE = !!certificate || !!building?.custom_data?.calificacion || docList.some(d => {
            const t = (d.file_name || '').toLowerCase();
            const meta = this.safeParseMetadata(d.metadata);
            const type = (meta?.document_type || '').toLowerCase();
        ...
        const normativeStatusEPBD = ['A', 'B', 'C', 'D'].includes(currentState.energy_class) ? 'compliant' :
            ['E', 'F'].includes(currentState.energy_class) ? 'partial' : 'non_compliant';
        const normativeStatusCEE = hasCEE ? 'compliant' : 'non_compliant';
        const mevComplianceRatio = mevsImplementedLength / mevs.length;
        const normativeStatusLey = mevComplianceRatio >= 0.5 ? 'compliant' :
            mevComplianceRatio >= 0.2 ? 'partial' : 'non_compliant';
        const normativeStatusCTE = ['A', 'B'].includes(currentState.energy_class) ? 'compliant' :
            ['C', 'D'].includes(currentState.energy_class) ? 'partial' : 'non_compliant';
        const normativeStatusRITE = (mevs.find(m => m.id === 'mev-03')?.status === 'implementada') ? 'compliant' :
            (mevs.find(m => m.id === 'mev-03')?.status === 'parcial') ? 'partial' : 'non_compliant';
        const normativeStatusIEE = (hasITE && hasCEE) ? 'compliant' :
            (hasITE || hasCEE) ? 'partial' : 'non_compliant';
        const normativeStatusEV = (mevs.find(m => m.id === 'mev-08')?.status === 'implementada') ? 'compliant' : 'non_compliant';
        const normativeStatusSRI = (mevs.find(m => m.id === 'mev-06')?.status === 'implementada') ? 'compliant' :
    ...
    analyzeMEVs(documents, certificate) {
        ...
        if (certificate?.rating === 'A' || certificate?.rating === 'B') {
            if (defaultMevs[0].status === 'no_implementada') {
                defaultMevs[0].status = 'implementada';
                defaultMevs[0].current_state = 'Aislamiento de alta eficiencia';
            }
            if (defaultMevs[1].status === 'no_implementada') {
                defaultMevs[1].status = 'implementada';
                defaultMevs[1].current_state = 'Ventanas eficientes';
            }
            if (defaultMevs[2].status === 'no_implementada') {
                defaultMevs[2].status = 'implementada';
                defaultMevs[2].current_state = 'Climatización óptima';
            }
            if (defaultMevs[4].status === 'no_implementada') {
                defaultMevs[4].status = 'parcial';
                defaultMevs[4].current_state = 'Paneles / Aerotermia parcial';
            }
            // Inferencia general para el resto
            defaultMevs.forEach(m => {
                if (m.status === 'no_implementada')
                    m.status = 'parcial';
            });
        }
        return defaultMevs;
...
exports.RegulatoryAuditService = RegulatoryAuditService;
//# sourceMappingURL=regulatoryAuditService.js.map