...
exports.signUpUserWithInvitation = signUpUserWithInvitation;
 ...
 */
async function signUpUserWithInvitation(params) {
    const { email, password, fullName, role, invitationToken } = params;
    const supabase = (0, supabase_1.getSupabaseClient)();
    const userService = new userService_1.UserService();
    const invitationService = new invitationService_1.InvitationService();
    // 1) Crear usuario en Auth
    const { data: authData, error: authError } = await supabase.auth.admin.createUser({
        email,
        password,
