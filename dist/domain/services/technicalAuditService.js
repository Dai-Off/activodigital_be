...
exports.TechnicalAuditService = void 0;
...
class TechnicalAuditService {
     ...
     */
    calculateCompletionPercentage(digitalBook, certificate, esgResult) {
        let score = 0;
        let maxScore = 0;
        ...
        if (digitalBook) {
            ...
            const technicalSections = [
                libroDigital_1.SectionType.MAINTENANCE_AND_CONSERVATION,
                libroDigital_1.SectionType.FACILITIES_AND_CONSUMPTION,
                libroDigital_1.SectionType.RENOVATIONS_AND_REHABILITATIONS,
                libroDigital_1.SectionType.SUSTAINABILITY_AND_ESG
            ];
            const sections = digitalBook.sections || [];
            const technicalComplete = sections.filter((s) => technicalSections.includes(s.type) && s.complete).length;
     ...
     */
    generateTasks(digitalBook, certificate, esgResult, building) {
        const tasks = [];
        let taskId = 1;
        ...
        if (!digitalBook) {
            ...
            const maintenanceSection = sections.find((s) => s.type === libroDigital_1.SectionType.MAINTENANCE_AND_CONSERVATION);
            ...
            const facilitiesSection = sections.find((s) => s.type === libroDigital_1.SectionType.FACILITIES_AND_CONSUMPTION);
            ...
            const renovationsSection = sections.find((s) => s.type === libroDigital_1.SectionType.RENOVATIONS_AND_REHABILITATIONS);
            ...
            const sustainabilitySection = sections.find((s) => s.type === libroDigital_1.SectionType.SUSTAINABILITY_AND_ESG);
     ...
     */
    calculatePotentialSavings(certificate, improvements) {
        if (!certificate) {
            return 0;
        }
        // Sumar ahorros de mejoras prioritarias (high y medium)
        const priorityImprovements = improvements.filter(imp => imp.priority === 'high' || imp.priority === 'medium');
