...
import {
  ...
  DocumentExpirationAlertsResponse,
  ExpirationAlertFilters,
} from "../../types/documentExpirationAlert";
import { BuildingDocument } from "../../types/buildingDocument";
import { UnitDocument } from "../../types/unitDocument";

export class DocumentExpirationAlertService {
  private getSupabase() {
   ...
   */
  private calculateAlertLevel(daysUntilExpiration: number): 'critical' | 'warning' | 'info' {
   ...
   */
  private async getBuildingDocumentsExpiring(
    daysAhead: number,
   ...
   */
  private async getUnitDocumentsExpiring(
    daysAhead: number,
    includeExpired: boolean,
    buildingId?: string,
    unitId?: string,
   ...
   */
  private async getServiceInvoicesExpiring(
    daysAhead: number,
    includeExpired: boolean,
    buildingId?: string
  ): Promise<DocumentExpirationAlert[]> {
    ...
    for (const invoice of data || []) {
      if (!invoice.expiration_date) continue;
  ...
  async getExpiringDocuments(
  ...
  ): Promise<DocumentExpirationAlertsResponse> {
    const {
      days_ahead = 90,
    ...
    const buildingAlerts = await this.getBuildingDocumentsExpiring(
      days_ahead,
      include_expired,
      building_id,
      category
    ...
    const unitAlerts = await this.getUnitDocumentsExpiring(
      days_ahead,
      include_expired,
      building_id,
      unit_id,
    ...
    const serviceInvoiceAlerts = await this.getServiceInvoicesExpiring(
      days_ahead,
      include_expired,
      building_id
    );
   ...
   */
  async findDocumentsExpiringSoon(): Promise<{
    total: number;
    building_documents: number;
    unit_documents: number;
