...
import { CatastroApiService } from "./catastroApiService";
import { BuildingUnit, CreateBuildingUnitRequest } from "../../types/edificio";
...
export class BuildingUnitService {
  ...
  private catastro = new CatastroApiService();

  async listUnits(buildingId: string): Promise<BuildingUnit[]> {
   ...
   */
  calculateOccupancy(units: BuildingUnit[]): number | null {
  ...
  async upsertUnits(
    buildingId: string,
    units: CreateBuildingUnitRequest[]
  ...
  private mapToUnit = (row: any): BuildingUnit => ({
    id: row.id,
