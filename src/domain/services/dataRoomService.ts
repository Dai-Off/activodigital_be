...
export class DataRoomService {
  ...
  async getAuditStatus(buildingId: string) {
   ...
   */
  async generateDossier(buildingId: string): Promise<Buffer> {
    const supabase = this.getSupabase();
    ...
    for (const record of records) {
      ...
      if (downloadError || !fileData) {
        console.warn(
          `No se pudo descargar ${record.storage_path}:`,
          downloadError,
        );
        continue;
      }
      ...
      try {
        if (mimeType === "application/pdf") {
          ...
          page.drawText(
            `Documento: ${record.file_name ?? record.checklist_id}`,
            {
              x: 50,
              y: 420,
              size: 14,
          ...
          page.drawText(
            `(Tipo de archivo no soportado para previsualización: ${mimeType})`,
            {
              x: 50,
              y: 395,
              size: 10,
