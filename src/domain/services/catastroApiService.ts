export class CatastroApiService {
  ...
  async checkHealth(): Promise<{
    online: boolean;
    latencyMs: number;
    status: "ok" | "timeout" | "error_http" | "falso_200" | "error_red" | "forzado_offline";
  }> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[CatastroApiService] Health check: HTTP ${response.status}`
        );
      ...
      if (contieneError) {
        console.warn(
          "[CatastroApiService] Health check: falso 200 detectado →",
      ...
      try {
        ...
        if (!tieneProvincias) {
          console.warn(
            "[CatastroApiService] Health check: respuesta JSON sin provincias →",
            bodyText.substring(0, 200)
          );
          return { online: false, latencyMs, status: "falso_200" };
        }
        ...
        console.warn(
          "[CatastroApiService] Health check: body no es JSON válido →",
          bodyText.substring(0, 200)
        );
        return { online: false, latencyMs, status: "falso_200" };
      }
      ...
      if (error?.name === "AbortError") {
        console.warn(
          `[CatastroApiService] Health check: timeout (${TIMEOUT_MS}ms)`
        );
