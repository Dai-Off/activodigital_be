...
export class ServiceInvoiceService {
  ...
  async getServiceInvoicesByBuilding(
    buildingId: string,
    userAuthId: string,
    serviceType?: string,
    year?: number,
    ...
    if (month) {
      invoices = invoices.filter(
        (inv) => new Date(inv.invoice_date).getMonth() + 1 === month
      );
    }
  ...
  async updateServiceInvoice(
    id: string,
    data: UpdateServiceInvoiceRequest,
    userAuthId: string
  ): Promise<ServiceInvoice | null> {
    ...
    if (data.amount_eur !== undefined) updateData.amount_eur = data.amount_eur;
    ...
    if (error) {
      ...
      throw new Error(
        `Error al actualizar factura de servicio: ${error.message}`
      );
    }
  ...
  async deleteServiceInvoice(id: string, userAuthId: string): Promise<void> {
