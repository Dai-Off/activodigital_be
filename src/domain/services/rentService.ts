...
export class RentService {
  ...
  async createRentInvoice(
    data: CreateRentInvoiceRequest,
    userAuthId: string
  ): Promise<RentInvoice> {
    const totalAmount =
      data.rentAmount + (data.additionalCharges || 0);
  ...
  async getRentInvoicesByBuilding(
    buildingId: string,
    userAuthId: string
  ): Promise<RentInvoice[]> {
  ...
  async deleteRentInvoice(id: string, userAuthId: string): Promise<void> {
  ...
  async getMonthlyRentSummary(
    buildingId: string,
    month: string, // YYYY-MM
    userAuthId: string
  ): Promise<MonthlyRentSummary> {
    const invoices = await this.getRentInvoicesByMonth(
      buildingId,
      month,
      userAuthId
    );
    ...
    const totalCollected = invoices.reduce(
      (sum, inv) => sum + (inv.paymentAmount || 0),
      0
    );
    ...
    const pendingAmount = pendingInvoices.reduce(
      (sum, inv) => sum + (inv.totalAmount - (inv.paymentAmount || 0)),
      0
    );
  ...
  private mapToRentInvoice(row: any): RentInvoice {
    return {
      id: row.id,
