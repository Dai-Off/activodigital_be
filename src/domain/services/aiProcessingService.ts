...
---`,
  ...
  async generateLicenciaDraft(
    ...
    try {
      ...
      const findValue = (keys: string[], defaultValue = '') => {
...
☐ Obras de ampliación, modificación, reforma o rehabilitación que no alteren estructura ni volumen`;
      ...
      if (statusSummary.length > 0) {
        documentCheckboxes = statusSummary.map((item: any) => {
...
*En cumplimiento del Reglamento General de Protección de Datos (RGPD), se le informa que sus datos personales serán tratados por el Ayuntamiento con el único fin de tramitar y verificar su declaración responsable urbanística.*`;
      ...
      const doc = h(Document, null,
        h(Page, { size: 'A4', style: styles.page },
          h(Text, { style: styles.header }, 'Borrador Oficial - Declaración Responsable Urbanística (DR)'),
          ...pdfComponents,
          h(Text, { style: styles.footer }, `ActivoDigital - Documento autogenerado en base a datos catastrales e inputs municipales - ${fechaActual}`)
        )
      );
