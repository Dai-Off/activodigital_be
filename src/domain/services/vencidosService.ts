...
export class VencidosService {
   ...
   */
  async getKPIsResumen(): Promise<KPIsResumen> {
    ...
    documentos.forEach((doc: any) => {
      const contenido: ContenidoExtraido = typeof doc.contenido_extraido === 'string'
        ? JSON.parse(doc.contenido_extraido)
        : doc.contenido_extraido;

      if (contenido.estado !== 'VENCIDO') return;
    ...
    const totalVencidos = documentos.filter((doc: any) => {
      const contenido: ContenidoExtraido = typeof doc.contenido_extraido === 'string'
        ? JSON.parse(doc.contenido_extraido)
        : doc.contenido_extraido;
   ...
   */
  async getConteosPorCategoria(): Promise<CategoriaConteo[]> {
    const supabase = this.getSupabase();
    ...
    const categorias: CategoriaConteo[] = Object.entries(conteos).map(([nombre, cantidad]) => ({
      nombre,
      cantidad
    }));
   ...
   */
  async getListadoVencidos(filtros: FiltrosVencidos): Promise<ListadoVencidosResponse> {
    const supabase = this.getSupabase();
    ...
    let documentosVencidos = todosDocumentos
      .map((doc: any) => this.mapToDocumentoVencido(doc))
      .filter((doc: DocumentoVencido) => {
        const contenido = doc.contenido_extraido;
        
        // Solo documentos vencidos
        if (contenido.estado !== 'VENCIDO') return false;

        // Filtro por prioridad
        if (filtros.prioridad && filtros.prioridad !== 'todas') {
          if (doc.prioridad_calculada !== filtros.prioridad) return false;
        }

        // Filtro por categoría
        if (filtros.categoria) {
          const categoriaDoc = this.obtenerCategoria(doc);
          if (categoriaDoc !== filtros.categoria) return false;
        }

        // Filtro por tipo de documento
        if (filtros.tipo_documento) {
          if (doc.tipo_documento !== filtros.tipo_documento) return false;
        }
        ...
        if (filtros.search) {
          ...
          if (!matchTitulo && !matchResumen && !matchBuilding) return false;
        }
   ...
   */
  async getDetalleItem(id: string): Promise<DocumentoVencido | null> {
    const supabase = this.getSupabase();
   ...
   */
  private mapToDocumentoVencido(doc: any): DocumentoVencido {
    const contenido: ContenidoExtraido = typeof doc.contenido_extraido === 'string'
      ? JSON.parse(doc.contenido_extraido)
      : doc.contenido_extraido;
   ...
   */
  private calcularPrioridad(contenido: ContenidoExtraido): 'alta' | 'media' | 'baja' {
    // Si tiene prioridad explícita
    if (contenido.prioridad) {
      const prioridadLower = contenido.prioridad.toLowerCase();
      if (prioridadLower.includes('alta') || prioridadLower.includes('high')) return 'alta';
      if (prioridadLower.includes('media') || prioridadLower.includes('medium')) return 'media';
      if (prioridadLower.includes('baja') || prioridadLower.includes('low')) return 'baja';
    }
    ...
    if (contenido.vigencia) {
      ...
      if (diasVencido > 180) return 'alta';
      if (diasVencido > 90) return 'media';
      return 'baja';
    }
   ...
   */
  private obtenerCategoria(doc: DocumentoVencido): string {
    const categoriaMap: { [key: string]: string } = {
      'Certificado': 'Certificados',
      'Certificados': 'Certificados',
      'Contrato': 'Contratos',
   ...
   */
  private parsearImporte(importeStr: string): number {
  ...
  async getFiltrosDisponibles(): Promise<FiltrosDisponibles> {
    ...
    const tiposDocumentoSet = new Set<string>();
