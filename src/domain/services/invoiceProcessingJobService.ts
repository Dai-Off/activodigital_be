...
import type {
  ...
  CreateInvoiceJobInput,
  InvoiceJobStatus,
} from '../../types/invoiceProcessingJob';

export class InvoiceProcessingJobService {
  private getSupabase() {
    return getSupabaseClient();
  }

  async create(input: CreateInvoiceJobInput): Promise<InvoiceProcessingJob> {
    ...
    if (error) throw new Error(`Error al crear job: ${error.message}`);
    return this.mapRow(data);
  }

  async getById(id: string): Promise<InvoiceProcessingJob | null> {
    const { data, error } = await this.getSupabase()
      .from('invoice_processing_jobs')
      .select('*')
      .eq('id', id)
  ...
  async getByIdForUser(id: string, userId: string): Promise<InvoiceProcessingJob | null> {
    const { data, error } = await this.getSupabase()
      .from('invoice_processing_jobs')
      .select('*')
      .eq('id', id)
  ...
  private mapRow(row: any): InvoiceProcessingJob {
    return {
      id: row.id,
      user_id: row.user_id,
