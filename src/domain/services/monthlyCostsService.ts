...
export class MonthlyCostsService {
  ...
  async getMonthlyCostsByBuilding(
    buildingId: string,
    userAuthId: string,
    year?: number,
    month?: number
  ...
  async getMonthlyCostsSummary(
    ...
    const total_annual_eur = costs.reduce(
      (sum, cost) => sum + (cost.total_monthly_eur || 0),
      0
    );
    const average_monthly_eur = total_annual_eur / costs.length;

    const breakdown = costs.reduce(
      (acc, cost) => ({
  ...
  private mapToMonthlyCost(dbRow: any): MonthlyCost {
    return {
      id: dbRow.id,
