...
import { getTrazabilityMap, responseListTrazability, TrazabilityCounts, TrazabilityMap, TrazabilityServiceParams, ActionsValues, ModuleValues } from './interfaceTrazability';
...
export class TrazabilityService {
    ...
    private async getGroupedActionCounts(): Promise<TrazabilityCounts> {
        ...
        const datos = data.reduce((acc: TrazabilityCounts, current: any) => {
            const actionName = current.action;
            acc[actionName] = (acc[actionName] || 0) + 1;
            return acc;
        }, {} as TrazabilityCounts);

        return datos;
    }

    private async getActionCount(actionString: string): Promise<number> {
        ...
        for (const actionName in counts) {
            if (Object.prototype.hasOwnProperty.call(counts, actionName)) {
                ...
                if (normalizedAction.includes(normalizedActionParam)) totalUpdateCount += counts[actionName];
            }
        }
    ...
    async listTrazability(): Promise<responseListTrazability> {
        ...
        if (error) throw new Error(`Error al listar trazabilidad: ${error.message}`);
        if (errorsUsers) throw new Error(`Error al listar trazabilidad: ${errorsUsers.message}`);
    ...
    private mapToTrazabilityWithRelations(row: any): getTrazabilityMap {
        return {
            id: row.id,
            action: row.action,
            module: row.module,
