    ...
    try {
      ...
      try {
        if (req.file.mimetype === "application/pdf") {
          ...
          res.status(400).json({
            error:
              "Formato de archivo no soportado. Solo se aceptan PDF y archivos de texto.",
      ...
      let sections;
      try {
        sections = await this.aiProcessingService.processDocumentText(
          documentText
        );
      ...
      try {
        console.log(
          "Secciones antes de crear libro:",
        ...
        console.log(
          "Secciones del libro creado:",
          JSON.stringify(book.sections, null, 2)
        );

        // Extraer y guardar campos ambientales de la sección de sostenibilidad
        const sustainabilitySection = sections.find(
          (s) => s.type === "sustainability_and_esg"
        );
        if (sustainabilitySection && sustainabilitySection.content) {
          console.log(
            "Actualizando campos ambientales desde sección de sostenibilidad..."
          );
          try {
