...
export class BuildingScenariosController {
   ...
   */
  simulateRehab = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const request: RehabSimulateRequest = req.body;
   ...
   */
  runCashflow = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        res.status(401).json({ error: 'Usuario no autenticado' });
      ...
      const { id: buildingId } = req.params;
      const request: CashflowRunRequest = req.body;
   ...
   */
  calculateNPV = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        res.status(401).json({ error: 'Usuario no autenticado' });
      ...
      const { id: buildingId } = req.params;
      const request: NpvRequest = req.body;
   ...
   */
  calculateIRR = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        res.status(401).json({ error: 'Usuario no autenticado' });
      ...
      const { id: buildingId } = req.params;
      const request: IrrRequest = req.body;
   ...
   */
  calculateSensitivity = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        res.status(401).json({ error: 'Usuario no autenticado' });
      ...
      const { id: buildingId } = req.params;
      const request: SensitivityRequest = req.body;
